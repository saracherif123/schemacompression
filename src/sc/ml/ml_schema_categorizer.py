...
class SchemaCategorizer:
    ...
    def __init__(self, model_type='random_forest'):
        """
        Initialize schema categorizer with specified model type.
        
        Args:
            model_type: One of 'random_forest', 'gradient_boosting', 'svm', 
...
def compare_all_models(schema_data):
    """
    Train and compare all ML models to generate performance comparison table.
    This matches the model comparison in the paper (Table 7).
    
    Args:
        schema_data: Dictionary with schema_name -> {schema_content, success_rate}
    ...
    categorizer = SchemaCategorizer()
    X, y = categorizer.prepare_training_data(schema_data)
    ...
    if n_samples < 2:
        raise ValueError(f"Insufficient data: Only {n_samples} sample(s) available. Need at least 2 samples to train models.")
    
    if unique_classes < 2:
        raise ValueError(f"Insufficient class diversity: Only {unique_classes} unique class(es) found. Need at least 2 different classes to train a classifier.")
    
    # Determine appropriate CV folds based on sample size
    if n_samples < 5:
        cv_folds = max(2, n_samples)  # Use leave-one-out or 2-fold for small datasets
    ...
    for model_type, model_name in models_to_compare:
        ...
        categorizer = SchemaCategorizer(model_type=model_type)
        categorizer.scaler = scaler  # Use same scaler
        categorizer.feature_extractor = SchemaFeatureExtractor()
        
        # Train
        categorizer.model.fit(X_scaled, y)
        
        # Cross-validation
        cv_scores = cross_val_score(categorizer.model, X_scaled, y, cv=cv_folds, scoring='accuracy')
        cv_f1 = cross_val_score(categorizer.model, X_scaled, y, cv=cv_folds, scoring='f1_macro')
        cv_precision = cross_val_score(categorizer.model, X_scaled, y, cv=cv_folds, scoring='precision_macro')
        ...
        print(f"  Accuracy: {accuracy:.1f}% (+/- {cv_scores_array.std() * 2 * 100:.1f}%)")
...
def main():
    ...
    print("Recommended method: SIMPLE compression (14.4% token reduction)")
